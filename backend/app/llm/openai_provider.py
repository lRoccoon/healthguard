...
Supports the standard OpenAI Chat Completions API and the Responses API.
Works with any OpenAI-compatible endpoint (e.g., OpenAI, Azure, vLLM, etc.).
"""
...
class OpenAIProvider(LLMProvider):
    ...
    async def chat_completion(
        ...
        **kwargs
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            logger.debug(
                f"LLM API call starting: provider=openai, model={payload['model']}, "
        ...
        **kwargs
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            if messages:
                ...
                message_summary += f", first: {first_msg}"
            logger.debug(
                f"LLM API stream starting: provider=openai, model={payload['model']}, "
                f"temperature={payload['temperature']}, {message_summary}"
            )
    ...
    async def responses(
        ...
        **kwargs
        ...
        The Responses API uses a slightly different input format.
        """
        start_time = time.time()
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            if messages:
                ...
                message_summary += f", first: {first_msg}"
            logger.debug(
                f"LLM API call starting (responses API): provider=openai, model={payload['model']}, "
                f"temperature={payload['temperature']}, {message_summary}"
            )
